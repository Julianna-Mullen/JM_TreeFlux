...
message("Initial dataset rows: ", nrow(my_data))
...
# CO2 fluxes less than zero are physically impossible and probably mean
# a measurement or instrument problem. Drop 'em
co2_lt0 <- my_data$CO2_lin_flux.estimate < 0
message("Dropping ", sum(co2_lt0), " negative CO2 fluxes")

# CO2 is normally low-noise and a low R2 model fit indicates a problem
# Drop those values
co2_lowr2 <- my_data$CO2_lin_r.squared < 0.75
message("Dropping ", sum(co2_lowr2), " low-R2 CO2 fluxes")

my_data |> filter(!co2_lt0, !co2_lowr2) -> my_data
message("Final dataset rows: ", nrow(my_data))
```

## Prep Soil Temp Data
```{r}
soil_temp_data <- read_L2_variable(
  variable = "soil-temp-5cm",
  path = "Sensor_Data/",
  quiet = TRUE
)
...
ggplot(
  ...
  facet_wrap(~Site) +
  labs(
    title = "Daily Mean Soil Temperature by Plot",
    x = "Date",
...
my_data %>%
  ...
  facet_grid(Species ~ .) +
  labs(
    title = "CH4 Flux by Species (2021–2025)",
    x = "Year",
...
my_data %>%
  ...
  facet_grid(Species ~ .) +
  labs(
    title = "CO2 Flux by Species (2021–2025)",
    x = "Year",
    y = "CO2 Flux (µmol/m2/s)",
    fill = "Year"
  ) +
  theme(legend.position = "right")
```
...
```{r}
ggplot(my_data, aes(
  x = Date, y = CH4_lin_flux.estimate,
  group = ID, color = Plot
)) +
  geom_line(alpha = 0.75) +
  facet_wrap(~Species) +
  labs(
    title = "CH4 Flux Over Time by Species and Plot",
    x = "Date",
    y = "CH4 Flux (nmol/m2/s)"
  )
...
ggplot(heat_data_CH4, aes(x = Year, y = Species, fill = mean_flux)) +
  ...
  labs(
    title = "Mean CH4 Flux by Species and Year", x = "Year", y = "Species",
    fill = "Mean CH4 Flux\n(nmol/m2/s)"
  ) +
  theme_minimal()
...
ggplot(my_data, aes(x = CO2_lin_flux.estimate, y = CH4_lin_flux.estimate, color = Species)) +
  ...
  labs(
    title = "Relationship Between CO2 and CH4 Flux by Species",
    x = "CO2 Flux (µmol/m2/s)", y = "CH4 Flux (nmol/m2/s)",
    color = "Species"
  ) +
  theme_minimal()
```
...
ggplot(
  ...
  facet_wrap(~Species) +
  labs(
    title = "Mean CH4 Flux by Species and Plot (2021–2026)",
    subtitle = "(No S7)",
    x = "Year", y = "Mean CH4 Flux (nmol/m2/s)"
  )
...
ggplot(
  ...
  facet_wrap(~Species) +
  labs(
    title = "Mean CO2 Flux by Species and Plot (2021–2026)",
    subtitle = "(No S7)",
    x = "Year",
    y = "Mean CO2 Flux (µmol/m2/s)"
  )
```
...
ggplot(
  ...
  facet_wrap(~Species) +
  labs(
    title = "Quarterly CO2 Flux with Flood Events Highlighted",
    x = "Year-Quarter",
    y = "CO2 Flux (µmol/m2/s)",
    shape = "Flood event"
  ) +
...
ggplot(
  ...
  facet_wrap(~Species) +
  labs(
    title = "Quarterly CH4 Flux with Flood Events Highlighted",
    x = "Year-Quarter",
    y = "CH4 Flux (nmol/m2/s)",
    shape = "Flood event"
  ) +
...
my_data %>%
  ...
  labs(
    y = "CH4 Flux (nmol/m2/s)",
    title = "Red Maple CH4 Flux by Individual Tree"
  )
```
...
ggplot(quarterly_CH4_S7, aes(x = YearQuarter, y = mean_flux)) +
  ...
  labs(
    title = "S7 Quarterly CH4 Flux with Flood Periods Highlighted",
    x = "Year-Quarter",
    y = "CH4 Flux (nmol/m2/s)"
  ) +
  theme_minimal() +
...
ggplot() +
  ...
  facet_wrap(~Species) +
  labs(
    title = "Quarterly CH4 Flux",
    subtitle = "(S7 excluded from means, shown separately)",
    x = "Year-Quarter",
    y = "CH4 Flux (nmol/m2/s)",
    shape = "Flood event"
  ) +
...
ggplot() +
  ...
  geom_point(
    data = quarterly_CH4_nos7, aes(
      x = YearQuarter,
      y = mean_flux, color = Plot
    )
  ) +
  geom_point(
    data = flood_plot_data,
    aes(x = YearQuarter, y = mean_flux, shape = Event),
    color = "black",
    size = 3
  ) +
  facet_wrap(~Species) +
  labs(
    title = "Quarterly CH4 Flux with Flood Events Across Treatments",
    x = "Year-Quarter",
    y = "CH4 Flux (nmol/m2/s)",
    shape = "Flood event"
  ) +
  scale_shape_manual(values = c(T1 = 17, T2 = 17, T3 = 17, T4 = 17)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
ggplot(my_data, aes(
  ...
  labs(
    y = "CH4 Flux (nmol/m2/s)",
    title = "Soil Temperature × Treatment Effects on CH4 Flux"
  )
...
ggplot(my_data, aes(
  ...
  labs(
    y = "CO2 Flux (µmol/m2/s)",
    title = "Soil Temperature × Treatment Effects on CO2 Flux"
  )
```
...
ggplot(my_data, aes(soil_temp_daily_mean, CH4_lin_flux.estimate)) +
  ...
  labs(
    y = "CH4 Flux (nmol/m2/s)",
    title = "Nonlinear Temperature Response of CH4"
  )
```
...
ggplot(my_data, aes(soil_temp_daily_mean, CO2_lin_flux.estimate)) +
  ...
  labs(
    y = "CO2 Flux (µmol/m2/s)",
    title = "Nonlinear Temperature Response of CO2"
  )
```
...
for (plot in unique(my_data$Plot)) {
  ...
  p <- ggplot(
    ...
    facet_wrap(~ID, scales = "free") +
    labs(
      title = paste0("Carbon Dioxide Flux vs. Daily Mean Soil Temperature (", plot, " Trees)"),
      x = "Daily Mean Soil Temperature (°C)",
      y = "CO2 Flux (µmol/m2/s)"
    )
  print(p)
...
ch4_sensitivity <- my_data %>%
  ungroup() %>%
  filter(!is.na(CH4_lin_flux.estimate), !is.na(soil_temp_daily_mean)) %>%
  filter(n() >= 3, .by = ID) %>%
  reframe(
...
CO2_sensitivity <- my_data %>%
  ungroup() %>%
  filter(!is.na(CO2_lin_flux.estimate), !is.na(soil_temp_daily_mean)) %>%
  filter(n() >= 3, .by = ID) %>%
  reframe(
...
```{r}
ggplot(
  subset(ch4_sensitivity, sensitivity == "Positive"),
  aes(
    x = reorder(ID, estimate),
    y = estimate
  )
) +
  ...
  coord_flip() +
  labs(
    title = "Positive Temperature Sensitivity (CH4)",
    x = "Tree ID",
    y = "Slope"
  )
```
```{r}
ggplot(
  subset(ch4_sensitivity, sensitivity == "Neutral"),
  aes(
    x = reorder(ID, estimate),
    y = estimate
  )
  ...
  coord_flip() +
  labs(
    title = "Neutral Temperature Sensitivity (CH4)",
    x = "Tree ID",
    y = "Slope"
  )
```
...
```{r}
ggplot(
  subset(CO2_sensitivity, sensitivity == "Positive"),
  aes(
    x = reorder(ID, estimate),
    y = estimate
  )
  ...
  coord_flip() +
  labs(
    title = "Positive Temperature Sensitivity (CO2)",
    x = "Tree ID",
    y = "Slope"
  )
```
```{r}
ggplot(
  subset(CO2_sensitivity, sensitivity == "Neutral"),
  aes(
    x = reorder(ID, estimate),
    y = estimate
  )
  ...
  coord_flip() +
  labs(
    title = "Neutral Temperature Sensitivity (CO2)",
    x = "Tree ID",
    y = "Slope"
  )
```
...
ggplot(
  ...
  labs(
    ...
    x = "Year-Quarter",
    y = "Treatment − Control CH4 Flux (nmol/m2/s)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
ggplot(
  ...
  labs(
    ...
    x = "Year-Quarter",
    y = "Treatment − Control CO2 Flux (µmol/m2/s)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
ggplot(flood_CH4_daily) +
  ...
  geom_rect(
    data = flood_date_relative,
    aes(
      xmin = day_start, xmax = day_end,
      ymin = -Inf, ymax = Inf
    ),
    fill = "lightgrey"
  ) +
...
cat(
  "CH4 range:",
  range(my_data$CH4_lin_flux.estimate, na.rm = TRUE), "\n"
)

cat(
  "CO2 range:",
  range(my_data$CO2_lin_flux.estimate, na.rm = TRUE), "\n"
)
```
# Species Level Q10
```{r}
