...
## Overall flux range (using boxplot) for each gas, by tree species and plot
```{r flux-ranges}
ggplot(my_data,
...
my_data %>%
  ...
  facet_wrap(~ ID) 
```
## S7 is a big emitter, to teh extent where it's fluxes throw off the dataset

# Separate S7 from all other trees (for Methane)
```{r}
s7_CH4 <- my_data %>%
  filter(ID == "S7") %>%
...
ggplot(my_data,aes(
                  x = soil_temp_daily_mean,
                   y = CO2_lin_flux.estimate,
                   color = Species) ) +
  geom_point(alpha = 0.5) + 
  # use a polynomial to produce a curved line
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), se = FALSE)
```

## GHG Flux x Soil Temp (colored by species)
```{r}
ggplot(my_data, aes(x = soil_temp_daily_mean,
             y = CH4_lin_flux.estimate,
             color = Species)) +
...
## Soil Temperature × Treatment Effects on GHG Flux
```{r}
ggplot(my_data, aes(x = soil_temp_daily_mean,
                  y = CH4_lin_flux.estimate,
                     color = Plot)) +
...
# Species Differences in Temperature Sensitivity
## Do different species have different average rates of response to temperature?
```{r}
...
# Relationship between Soil Temp and GHG Flux
```{r}
 ggplot(my_data,
...
for(plot in unique(my_data$Plot)) {
  ...
  ggplot(plot_data,
    ...
    facet_wrap(~ID, scales = "free") +
    labs(
      title = paste0("Methane Flux vs. Daily Mean Soil Temperature (", plot, " Trees)"),
                     x = "Daily Mean Soil Temperature (°C)",
                     y = "CH4 Flux")
}
```
...
```{r}
q10_results_list <- list()
for(id in unique(my_data$ID)) {

  q10_data <- my_data |> filter(ID == id)
  
# Compute Q10 using the method we went over
  fit <- lm(log(CH4_lin_flux.estimate) ~ soil_temp_daily_mean, data = q10_data)

x <- seq(min(q10_data$soil_temp_daily_mean),
         max(q10_data$soil_temp_daily_mean),
         length.out = 20)

y_pred <- predict(fit, newdata = data.frame(soil_temp_daily_mean = x))

q10 <- (y_pred[20] / y_pred[1]) ^ (10 / (x[20] - x[1]))
  # Save in our running list
  q10_results_list[[id]] <- q10
}
# Bind the list into a summary data frame
q10_results <- bind_rows(q10_results_list, .id = "ID")
```
