...
## Taking a closer look at the Red Maple trees to identify why the flux averages are so high
```{r}
my_data %>%
...
## S7 is a big emitter, to the extent where it's fluxes throw off the dataset
...
# Relationship between daily mean soil temperature and GHF flux
...
## three-panel time series figure that tracks methane flux, carbon dioxide flux, and soil temperature over time, summarized by quarter.
```{r}
...
# Methane x Soil Temp. by Plot (!there is an error to fix here!)
```{r}
# Generate plot-specific graphs
for(plot in unique(my_data$Plot)) {
  
  plot_data <- my_data %>%
    dplyr::filter(Plot == plot)
...
# Carbon Dioxide x Soil by Tree Species
